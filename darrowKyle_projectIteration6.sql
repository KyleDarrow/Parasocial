...
-- Second index from query 2.
CREATE INDEX UserQuestionIdx
ON UserQ(Question);

-- Third index from query 3.
...
--INSERTS - Not needed, but contains the examples I used in project.
...
-- Inserts user questions into UserQ.
...
-- Inserts AI answers into AIResponse.
BEGIN TRANSACTION AddAIAnswer
EXECUTE AddAIAnswer 6, 'I have been doing a lot of powerlifting and trying to tone my physique.';
EXECUTE AddAIAnswer 7, 'Anytime a horror movie or a mystery movie comes out, you will see me at the theaters!';
COMMIT TRANSACTION AddAIAnswer;



--TRIGGERS
CREATE TRIGGER AIResponseChangeTrigger
ON AIResponse
AFTER UPDATE
AS
BEGIN
	DECLARE @OldAnswer VARCHAR(1000) = (SELECT Answer FROM DELETED);
	DECLARE @NewAnswer VARCHAR(1000) = (SELECT Answer FROM INSERTED);

	IF (@OldAnswer <> @NewAnswer)
		INSERT INTO AIResponseChange (AnswerChangeID, OldAnswer, NewAnswer, AnswerID, DateChanged)
		VALUES(NEXT VALUE FOR answerchange_seq, @OldAnswer, @NewAnswer, (SELECT AnswerID FROM INSERTED), GETDATE());
END;

-- Test for trigger used in project.
UPDATE AIResponse
SET Answer = 'I recently took up rock climbing and have enjoyed doing that, its tough!'
WHERE AnswerID = 1;

UPDATE AIResponse
SET Answer = 'Rollerblading has eaten up a lot of my time recently, I love going fast!'
WHERE AnswerID = 1;
...
-- Fourth Query - How many normal vs. verified users use Parasocial?
-- This query answers this question.
SELECT Account.AccountType, COUNT(*) AS UserTypes
FROM Account
GROUP BY Account.AccountType
ORDER BY UserTypes; 

-- Fifth Query - What is the number of questions each user currently has on their account?
-- This query answers this question.
SELECT Account.AccountName, COUNT (*) AS TotalQuestions
FROM Account
JOIN Question ON Account.AccountID = Question.AccountID
GROUP BY Account.AccountName
ORDER BY TotalQuestions;


-- View all tables
-- Just to help with viewing tables if needed I added SELECT statements that will allow you to view every table.
